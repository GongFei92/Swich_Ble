...
public class BluetoothLeService extends Service {
	...
	private static String reConnectAddress;
			...
			final BluetoothGattCharacteristic characteristic)
	{
		...
		{
			final StringBuilder stringBuilder = new StringBuilder(data.length); //字符串变量 单线程
			for (byte byteChar : data) //byteChar是遍历后赋值的变量，data是要遍历的数组。
			{
				stringBuilder.append(String.format("%02X ", byteChar));//%02X：以十六进制输出,2为指定的输出字段的宽度.如果位数小于2,则左端补0
	...
	public void setReConnectAddress(final String address) {
